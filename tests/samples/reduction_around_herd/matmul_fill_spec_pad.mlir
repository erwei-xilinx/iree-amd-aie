...
module attributes { transform.with_named_sequence } {
  ...
  transform.named_sequence @__transform_main(%variant_op: !transform.any_op {transform.read_only}) {
    ...
    %padded_reduction_out = transform.get_producer_of_operand %padded_reduction[2] : (!transform.any_op) -> (!transform.any_op)
    %padded_reduction_out_buffer, %padded_reduction_out_new = transform.structured.bufferize_to_allocation %padded_reduction_out
        {memory_space = 1, bufferize_destination_only, emit_dealloc} : !transform.any_op
    ...
    %tiled_matmul_1, %forall_1 =
      transform.structured.tile_using_forall %padded_reduction tile_sizes [16, 16, 0]
    ...
    %padded_lhs = transform.get_producer_of_operand %padded_1[0] : (!transform.any_op) -> (!transform.any_op)
    %padded_lhs_buffer, %padded_lhs_new = transform.structured.bufferize_to_allocation %padded_lhs
        {memory_space = 2, bufferize_destination_only, emit_dealloc} : !transform.any_op
        
    %padded_rhs = transform.get_producer_of_operand %padded_1[1] : (!transform.any_op) -> (!transform.any_op)
