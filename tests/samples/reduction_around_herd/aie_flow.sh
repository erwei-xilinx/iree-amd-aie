...
${IREE_OPT} \
  ...
  --iree-codegen-transform-dialect-library=${SAMPLES_DIR}/reduction_around_herd/matmul_fill_spec_pad.mlir ${DEBUG_FLAGS} | \
${IREE_OPT} \
...
${IREE_OPT} \
  --pass-pipeline='builtin.module(hal.executable(hal.executable.variant(builtin.module(air-dependency, air-hoist-dma-in-accum-pattern, air-dma-to-channel, canonicalize, cse, air-dependency-canonicalize, canonicalize, cse))))' ${DEBUG_FLAGS} | \
  # --pass-pipeline='builtin.module(hal.executable(hal.executable.variant(builtin.module(air-dependency, air-dma-to-channel, canonicalize, cse, air-dependency-canonicalize, canonicalize, cse, air-label-scf-for-to-ping-pong))))' ${DEBUG_FLAGS} | \
${IREE_OPT} \
  --pass-pipeline='builtin.module(hal.executable(hal.executable.variant(builtin.module(air-fuse-channels, canonicalize, cse, air-isolate-async-dma-loop-nests, air-label-scf-for-in-segment, air-unroll-loop-for-pipelining-pattern))))' ${DEBUG_FLAGS} | \
  # --pass-pipeline='builtin.module(hal.executable(hal.executable.variant(builtin.module(air-ping-pong-transform{keep-memref-dealloc=true}, canonicalize, cse, air-fuse-channels, canonicalize, cse, air-isolate-async-dma-loop-nests, air-label-scf-for-in-segment, air-unroll-loop-for-pipelining-pattern))))' ${DEBUG_FLAGS} #| \
${IREE_OPT} \
